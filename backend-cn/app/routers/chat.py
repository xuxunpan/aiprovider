            ...
            try:
                async for evt in hk_chat_client.stream_message(
                    ...
                    if etype == "delta":
                        ...
                        # HK 侧 rollout 丢失后已新建 thread，回写 DB 以便后续消息用新 thread
                        new_tid = evt.get("thread_id")
                        if new_tid:
                            logger.info(
                                "会话 thread 已由香港重建: session_id=%s old=%s new=%s",
                                session_id, thread_id, new_tid,
                            )
